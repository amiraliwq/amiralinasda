...
export default function SignupPage() {
  ...
  return <main dir="rtl" className="min-h-screen auth-bg px-4 py-10">
    <div className="mx-auto max-w-2xl rounded-[2rem] auth-card border-white/70 p-6 shadow-2xl backdrop-blur sm:p-10">
      ...
      <form onSubmit={submit} className="mt-8 grid gap-4 sm:grid-cols-2">
        <label>نام<input required className="mt-2 w-full auth-input rounded-2xl p-3" value={form.firstName} onChange={e=>set("firstName",e.target.value)}/></label>
        <label>نام خانوادگی<input required className="mt-2 w-full auth-input rounded-2xl p-3" value={form.lastName} onChange={e=>set("lastName",e.target.value)}/></label>
        <label>نام کاربری<input required minLength={3} className="mt-2 w-full auth-input rounded-2xl p-3" dir="ltr" value={form.username} onChange={e=>set("username",e.target.value)}/></label>
        <label>شماره تلفن
          <input required inputMode="tel" autoComplete="tel" className="mt-2 w-full auth-input rounded-2xl p-3" dir="ltr" placeholder="09123456789" value={form.phone} onChange={e=>set("phone",e.target.value)}/>
        ...
        <label>رمز عبور<input required type="password" minLength={8} autoComplete="new-password" className="mt-2 w-full auth-input rounded-2xl p-3" dir="ltr" value={form.password} onChange={e=>set("password",e.target.value)}/></label>
        <label>تکرار رمز عبور<input required type="password" autoComplete="new-password" className="mt-2 w-full auth-input rounded-2xl p-3" dir="ltr" value={form.confirm} onChange={e=>set("confirm",e.target.value)}/></label>
        {error&&<div className="sm:col-span-2 rounded-2xl bg-red-50 p-3 text-sm text-red-700">{error}</div>}
        <button disabled={busy} className="sm:col-span-2 rounded-2xl auth-button p-3 font-bold text-white shadow-lg disabled:opacity-50">{busy?"در حال ثبت‌نام...":"ایجاد حساب"}</button>
      </form>
